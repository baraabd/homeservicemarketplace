...
## التوافق مع تحديث develop المتزامن (#100)

دُمج #100 أثناء إعداد هذا الإصلاح، عند `6a0944a222be5d8e32d0d4735b25375899823567`.
يحافظ هذا الفرع على التحميل الديناميكي وشاشة الاستعادة وزر إعادة المحاولة،
وعلى المنفذ الصارم وأمر `dev:reset` والتشغيل المباشر دون بناء contracts
الزائد في أمر `dev`. تضاف إليه الحماية من ملف الإعداد القديم؛ لا يُلغى أي
اختبار من اختبارات #100 ولا تُستبدل تحسيناته.

إعادة إنتاج الإعداد القديم على النسخة السابقة أعطت شاشة بيضاء، موثقة في
تشغيل `36201630241` على Windows وLinux. بعد استيعاب #100، يؤدي الإعداد
القديم نفسه إلى خطأ الاستيراد ذاته ولكن تظهر شاشة الاستعادة بدل الصفحة
الفارغة. الاختبار السلبي الحالي يتحقق من خطأ الاستيراد المحدد ورسالة
الاستعادة وزرها. الاختبار الإيجابي يتحقق من ظهور التطبيق الحقيقي، وليس
الاكتفاء بوجود محتوى التحميل أو شاشة الاستعادة.
...
متعارضًا لإعادة إنتاج فشل الاستيراد مع ظهور شاشة الاستعادة. بعد ذلك يشغّل أمر `pnpm dev` الحقيقي
