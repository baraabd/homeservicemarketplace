...
a `JWT_ACCESS_SECRET` key assigned a quoted per-suite literal, in the Phase 3
integration specs.

> The offending line is described rather than quoted, deliberately. The first
> draft of this section reproduced it verbatim inside a code fence, and gitleaks
> flagged **the documentation** on the next run — a finding is a finding
> wherever the bytes live, and a write-up of a secret-scanning defect is the
> last place that should need a second pass.
...
### Defect 7 — a pixel-exact visual gate that no CI run could ever pass

The second run, on `76a6f92`, was the **first time `prototype-reference.spec.ts`
had ever executed in CI** — on `b03c405` that job was still running when the
fix push cancelled it. It failed 12 times: every viewport, both languages, all
of them the snapshot comparison.

The cause is a claim in `playwright.config.ts` that this sprint wrote and that
is wrong. `snapshotPathTemplate` drops the platform suffix, justified as:

> these captures are deterministic by construction (pinned icon script,
> vendored fonts, fixed viewport, animations off), so the platform suffix would
> only hide drift

Pinning removes every source of drift **above** the rasterizer. It does not
touch the rasterizer: Skia hints and antialiases glyphs differently on Linux
than on Windows, so the same DOM produces a different bitmap. The baselines
under `e2e/__screenshots__/reference/` were captured on the developer's Windows
host, so a Linux CI run cannot match them — and `toMatchSnapshot` is
byte-exact, unlike `toHaveScreenshot`, which this config does give an
antialiasing tolerance.

This is worth stating plainly: §4.5 gate 20 recorded "Playwright full matrix, 3
shards, exit 0 ×3 — 708 passed". That was true, and it was true **only on
Windows**. The local matrix could not have caught this, because the defect _is_
the difference between the two platforms.

Fixed by making the pixel-exact spec opt-in (`E2E_VISUAL_REFERENCE=1`) rather
than by loosening it. A pixel tolerance wide enough to absorb a different
rasterizer is wide enough to absorb the regressions the gate exists to catch,
and a `test.skip` would have made it silently do nothing — which this config's
own comments argue against in three places. Verified both ways:
`playwright test --list` reports **0** of these tests by default and **12**
with the flag set, which are exactly the 12 CI failed.

**Follow-up, not done here:** committing Linux baselines beside the Windows ones
would restore this as a real CI gate. It needs a Linux capture this host cannot
produce.

### Final CI results on `76a6f92`
...
| **CodeQL**                                            | **success**         |
| Browser E2E (Playwright / Chromium)                   | failure — defect 7  |
| Dependency, secret, and container scans               | failure — see §5.8  |
| CI gate                                               | failure (aggregate) |

`Integration & E2E` passing is the direct confirmation that the derived-secret
change of defect 5 did not disturb the Phase 3 journeys: they ran against real
Postgres and Redis with the gates on.

### CodeQL, stated precisely

The check went from `failure` — "4 new alerts including 1 high severity
security vulnerability" — to `success` — "1 new alert ... 1 medium". So the
gate blocks on high/critical and reports medium without blocking.

- **#9, high, `js/incomplete-url-substring-sanitization` — fixed.**
- **#10, #11, #12, medium, `js/http-to-file-access` — fixed**, but they
  consolidated into **one new medium, #13**, at
  `vendor-prototype-assets.mjs:91`: the single `writeFileSync` inside
  `writeVerified()`. Three write sites became one, and that one still writes
  downloaded bytes to disk — which is the script's purpose — now only after the
  bytes match the SHA-256 pinned in `manifest.json`.

**Alert #13 is open, and deliberately not dismissed.** It is a medium,
non-blocking finding in a developer-only vendoring tool that is never built,
shipped or executed by the application, and the flow it describes is intended
and now integrity-gated. Dismissing a real finding is a decision for a human
reviewer, not for the author of the code it concerns. Nothing in this sprint
suppressed, baselined or downgraded a CodeQL result: **0 alerts dismissed.**

## 5.8 Why this work was republished on a clean branch

The secret-scan failure of defect 5 could not be closed by any further commit
on the original branch. `gitleaks-action` scans **every commit in the pull
request**, and the literals lived in `fb38bb4`, four commits back. Fixing them
at the tip left them in the history the scanner reads, so the gate stayed red
no matter how correct the final tree was.

Three routes were considered and two were rejected on policy grounds, not
convenience:

| Route                             | Rejected because                                                                                                                                                                                                            |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Rewrite the branch and force-push | `.claude/rules/provider-onboarding-v2.md` says **"Never force-push."** A rule that only holds when it is inconvenient is not a rule.                                                                                        |
| Baseline the 7 fingerprints       | `.gitleaksignore` restricts itself to individually reviewed **historical** fixtures and warns, in its own header, against exactly this. Adding this week's commits would hollow out the one control that caught the defect. |
| **Clean branch from `develop`**   | **Chosen.** Nothing published is rewritten, nothing is force-pushed, and no secret-shaped literal exists in any commit of the new range.                                                                                    |

**Method.** The final tree was captured on a local ref (`snapshot/9b29-final`),
a new branch was created at the exact live `origin/develop` SHA `563cfe73`, and
`git read-tree -m -u` made its tree equal the snapshot. No commit from the
original branch is cherry-picked or carried. The transfer was proved rather
than assumed: immediately after the `read-tree`,
`git diff --stat snapshot/9b29-final` was **empty** — the two trees were
byte-identical.

This section is the one deliberate difference. It was appended afterwards,
because a document cannot describe its own republication and still be identical
to the tree that preceded it. Every other path in the commit is the verified
snapshot.

**What is preserved.** `fix/sprint-09b29-provider-onboarding-v2-parity` still
points at `76a6f92` locally _and_ on the remote, and draft PR #72 stays open
until the replacement is confirmed. Its eleven purpose-separated checkpoints,
and the two CI runs that found defects 5, 6 and 7, remain readable there. The
four developer stashes were never touched at any point in this session.

**What is lost, stated plainly.** The replacement is a single squashed commit,
so the granular checkpoint history does not survive into `develop`. That is the
price of not rewriting a published ref, and PR #72 is the mitigation: the
checkpoints are still readable, they are simply not what gets merged. The
repository squash-merges every sprint PR anyway (`develop`'s history is
`(#66)`, `(#67)`, `(#68)`, `(#70)`, `(#71)`), so the merged shape is unchanged.

**Local verification before the commit was created**

| Gate                                                           | Result                              |
| -------------------------------------------------------------- | ----------------------------------- |
| `git diff --stat snapshot/9b29-final`                          | empty — trees byte-identical        |
