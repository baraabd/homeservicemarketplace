...
FOR EACH ROW EXECUTE FUNCTION hsm_guard_published_plan_version();

-- Entitlements attached to a published plan version are part of the commercial snapshot.
CREATE FUNCTION hsm_guard_published_plan_entitlement() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE target_plan_version TEXT;
BEGIN
  target_plan_version := COALESCE(OLD."planVersionId", NEW."planVersionId");
...
FOR EACH ROW EXECUTE FUNCTION hsm_guard_published_plan_entitlement();
...
ALTER TABLE "PaymentIntent"
  ADD CONSTRAINT "payment_currency_upper" CHECK ("currency" ~ '^[A-Z]{3}$');

ALTER TABLE "EntitlementUsage"
  ADD CONSTRAINT "usage_period_valid" CHECK ("periodEnd" > "periodStart");

ALTER TABLE "LedgerTransaction"
  ADD CONSTRAINT "ledger_transaction_currency_upper" CHECK ("currency" ~ '^[A-Z]{3}$');
ALTER TABLE "LedgerEntry"
  ADD CONSTRAINT "ledger_entry_currency_upper" CHECK ("currency" ~ '^[A-Z]{3}$');

-- A ledger transaction has a strict forward-only lifecycle. Reversal is represented
-- by a separate compensating transaction linked through reversesTransactionId.
...
BEGIN
  ...
  END IF;
  IF OLD."status" IN ('POSTED','REVERSED') AND NEW."status" IS DISTINCT FROM OLD."status" THEN
    RAISE EXCEPTION 'posted ledger status is immutable';
  END IF;
  RETURN NEW;
END $$;
CREATE TRIGGER ledger_status_transition_guard
BEFORE UPDATE OF "status" ON "LedgerTransaction"
FOR EACH ROW EXECUTE FUNCTION hsm_guard_ledger_status_transition();

-- An entry must use the transaction currency and an account denominated in that currency.
...
FOR EACH ROW EXECUTE FUNCTION hsm_validate_ledger_entry_currency();
