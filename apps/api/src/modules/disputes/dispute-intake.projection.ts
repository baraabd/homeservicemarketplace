import {
  ...
  type ParticipantDisputeSummary, type ParticipantDisputeState,
...
const STATES: Record<IntakeCase['status'], ParticipantDisputeState> = {
  OPEN: 'SUBMITTED', IN_REVIEW: 'IN_REVIEW', CANCELLED: 'CLOSED',
  RESOLVED_REFUND: 'DECISION_RECORDED', RESOLVED_PARTIAL: 'DECISION_RECORDED',
  RESOLVED_DENIED: 'DECISION_RECORDED',
};
export function participantSummary(row: IntakeCase, actorUserId: string): ParticipantDisputeSummary {
  return {
    id: row.id,
