...
import {
  ...
  type CreateParticipantDisputeResponse,
  type DisputeBookingChoices,
  type DisputeIntakeContext,
  type ParticipantDisputeDetail,
  type ParticipantDisputeList,
...
export class DisputeIntakeService {
  constructor(
    ...
    @Optional() private readonly workspace?: DisputeWorkspaceService,
  ) {}

  async bookings(
    actorUserId: string,
    query: { limit?: number; cursor?: string },
  ): Promise<DisputeBookingChoices> {
  ...
  async context(actorUserId: string, bookingId: string): Promise<DisputeIntakeContext> {
    ...
    const { blocker, deadline } = intakeEligibility({
      ...
      terminalAt: await this.repository.terminalAt(booking),
      now: new Date(),
    });
    return {
      booking: {
      ...
      serverDrafts: !!this.workspace && (await this.workspace.available(actorUserId)),
      blocker: active
        ? active.id.startsWith(INTAKE_ID_PREFIX)
          ? 'ALREADY_OPEN'
          : 'LEGACY_REVIEW'
        : blocker,
  ...
  async list(
    actorUserId: string,
    query: { limit?: number; cursor?: string },
  ): Promise<ParticipantDisputeList> {
    const limit = Math.min(50, Math.max(1, query.limit ?? 20));
  ...
  async detail(actorUserId: string, id: string): Promise<ParticipantDisputeDetail> {
    ...
    return {
      ...summary,
      workspaceAvailable: !!this.workspace && (await this.workspace.exists(actorUserId, id)),
      issueCode: parsed.data.issueCode,
      requestedOutcome: parsed.data.requestedOutcome,
      statement:
        row.openedById === actorUserId
          ? (receipt?.message ??
            (this.workspace
              ? await this.workspace.originalStatement(actorUserId, id, receipt?.id)
              : null))
          : null,
      ...
      nextAction:
        summary.state === 'SUBMITTED' || summary.state === 'IN_REVIEW'
          ? 'WAIT_FOR_REVIEW'
          : 'CONTACT_SUPPORT',
  ...
  async create(
    actorUserId: string,
    input: CreateParticipantDisputeRequest,
  ): Promise<CreateParticipantDisputeResponse> {
    if (!validIntakeChoices(input.issueCode, input.requestedOutcome))
      throw new AppError(
        'VALIDATION_ERROR',
        'Choose a supported issue and requested outcome.',
        400,
      );
    ...
    if (statement.length < 20 || statement.length > 4000)
      throw new AppError(
        'VALIDATION_ERROR',
        'The statement must contain 20 to 4000 characters.',
        400,
      );
    ...
    try {
      return await this.transactions.run(async (tx) => {
        if (!(await this.repository.ownedBooking(input.bookingId, actorUserId, tx)))
          throw notFound();
        ...
        if (existingIntent) {
          ...
          const parsed = intakeReceiptSchema.safeParse(receipt?.after);
          if (
            existingIntent.deletedAt ||
            existingIntent.openedById !== actorUserId ||
            !parsed.success ||
            parsed.data.requestHash !== requestHash
          )
            throw new AppError(
              'CONFLICT',
              'The submission intent cannot be reused for different content.',
              409,
            );
          return {
            dispute: participantSummary(existingIntent, actorUserId),
            created: false,
            replayed: true,
          };
        }
        ...
        if (active) {
          if (!active.id.startsWith(INTAKE_ID_PREFIX))
            throw new AppError(
              'CONFLICT',
              'An existing support case requires a support review.',
              409,
            );
          return {
            dispute: participantSummary(active, actorUserId),
            created: false,
            replayed: false,
          };
        }
        const policy = parseIntakePolicy(
          (await this.settings.findByKey(DISPUTE_INTAKE_SETTING, tx))?.value,
        );
        const eligibility = intakeEligibility({
          policy,
          actorUserId,
          bookingState: booking.status,
          terminalAt: await this.repository.terminalAt(booking, tx),
          now: new Date(),
        });
        ...
        if (eligibility.blocker || !policy || input.policyVersion !== policy.revision)
          throw new AppError(
            'CONFLICT',
            'Booking eligibility or support policy has changed. Reload before submitting.',
            409,
          );
        ...
        await tx.disputeEvent.create({
          data: {
            ...
            after: {
              ...
              status: 'OPEN',
              origin: 'PARTICIPANT_INTAKE_V1',
            },
          },
        });
        const workspace = this.workspace
          ? await this.workspace.initialize(tx, id, actorUserId)
          : null;
        const recipient = this.counterparty(booking, actorUserId);
        let notificationId: string | null = null;
        if (recipient && !workspace) {
        ...
        await this.outbox.enqueue(
          {
            ...
            aggregateId: id,
            eventType: DISPUTE_INTAKE_EVENT,
        ...
        return {
          dispute: participantSummary({ ...row, booking, workspace }, actorUserId),
          created: true,
          replayed: false,
        };
      });
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002')
        throw new AppError(
          'CONFLICT',
          'A support case already exists. Reload the booking before continuing.',
          409,
        );
      throw error;
    }
  ...
  private counterparty(booking: IntakeBooking, actorUserId: string): string | null {
    const id =
      booking.seekerUserId === actorUserId ? booking.provider.userId : booking.seekerUserId;
