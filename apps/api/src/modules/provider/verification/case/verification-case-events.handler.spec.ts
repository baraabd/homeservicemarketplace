...
describe('VerificationCaseEventsHandler', () => {
  it('claims exactly the event types the workflow service emits', () => {
    ...
    expect([...handler.eventTypes].sort()).toEqual(
      [
        ...
        OutboxEventType.VERIFICATION_CASE_ACCESS_CLOSED,
      ].sort(),
    );
    ...
    expect(OutboxEventType.VERIFICATION_CASE_APPROVED).toBe('verification.case.approved');
    expect(OutboxEventType.VERIFICATION_CASE_ACCESS_CLOSED).toBe('verification.case.access_closed');
  });
  ...
  it.each(['SUBMITTED', 'ACTION_REQUIRED', 'IN_REVIEW', 'REJECTED', 'VERIFIED', 'EXPIRED'])(
