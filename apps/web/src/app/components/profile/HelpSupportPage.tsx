...
import {
  ChevronLeft, ChevronRight, Send, Check, CheckCheck,
  HelpCircle, CreditCard, Clock, Star, X,
} from "lucide-react";
...
function getBotReply(text: string, lang: string): string {
  ...
  const key =
    lower.includes("cancel") || lower.includes("إلغ")   ? "cancel"    :
    lower.includes("paid")   || lower.includes("مدفو")  ? "paid"      :
    lower.includes("price")  || lower.includes("سعر")   ? "price"     :
    lower.includes("satisf") || lower.includes("راض")   ? "satisfied" : "default";
...
const SEED_EN: Message[] = [
  { id: "b0", from: "bot",  text: "👋 Hi Ahmed! I'm your FixNow Support assistant. How can I help you today?", time: "9:00 AM", read: true },
  { id: "b1", from: "bot",  text: "You can ask me about bookings, payments, how the platform works, or anything else!", time: "9:00 AM", read: true },
];
const SEED_AR: Message[] = [
  { id: "b0", from: "bot",  text: "👋 مرحباً أحمد! أنا مساعد دعم فيكس ناو. كيف يمكنني مساعدتك اليوم؟",  time: "9:00 ص", read: true },
  { id: "b1", from: "bot",  text: "يمكنك سؤالي عن الحجوزات، المدفوعات، كيفية عمل المنصة، أو أي شيء آخر!", time: "9:00 ص", read: true },
];
...
export function HelpSupportPage({ onBack }: HelpSupportPageProps) {
  ...
  const sendMessage = (text: string) => {
    if (!text.trim()) return;
    const userMsg: Message = { id: `u${Date.now()}`, from: "user", text: text.trim(), time: now(), read: false };
    ...
    setTimeout(() => {
      ...
      setMessages((prev) => [
        ...prev,
        { id: `b${Date.now()}`, from: "bot", text: getBotReply(text, lang), time: now(), read: false },
      ]);
    }, 1400);
  };
  ...
  const FAQ_ICONS = [<HelpCircle size={13}/>, <CreditCard size={13}/>, <Clock size={13}/>, <Star size={13}/>];
    ...
    >
      ...
      <div className="flex-shrink-0 bg-white dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700 shadow-sm">
        <div className="flex items-center gap-3 px-4 py-3.5">
          ...
          <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center flex-shrink-0">
            <span className="text-white" style={{ fontSize: "12px", fontWeight: 800 }}>FN</span>
          </div>

          <div className="flex-1">
            <p className="text-slate-900 dark:text-white" style={{ fontSize: "15px", fontWeight: 700 }}>{L.support}</p>
            <div className="flex items-center gap-1">
              <div className="w-2 h-2 rounded-full bg-green-400" />
              <span className="text-slate-400 dark:text-slate-500" style={{ fontSize: "11px" }}>{L.online}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-2 px-4 pb-3 overflow-x-auto" style={{ scrollbarWidth: "none" }}>
          {FAQS.map((faq, i) => (
            ...
            >
              ...
              <span className="text-amber-700 dark:text-amber-300" style={{ fontSize: "11px", fontWeight: 600, whiteSpace: "nowrap" }}>
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-slate-400 dark:text-slate-500 bg-white dark:bg-slate-800 px-3 py-1 rounded-full border border-slate-200 dark:border-slate-700" style={{ fontSize: "11px" }}>
            {L.today}
          </span>
          <div className="flex-1 h-px bg-slate-200 dark:bg-slate-700" />
        </div>
            ...
            >
              {!isUser && (
                <div className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 mb-1 bg-gradient-to-br from-amber-400 to-orange-500">
                  <span className="text-white" style={{ fontSize: "10px", fontWeight: 800 }}>FN</span>
                </div>
              )}
        ...
        {messages.length > 4 && !rated && (
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-3 mb-3">
            <p className="text-slate-600 dark:text-slate-300 text-center mb-2" style={{ fontSize: "12px" }}>{L.rateChat}</p>
            <div className="flex justify-center gap-2">
              {[1,2,3,4,5].map((s) => (
                <button key={s} onClick={() => { setRating(s); setRated(true); }} className="active:scale-90 transition-all">
        ...
        {rated && (
          <div className="flex items-center justify-center gap-2 bg-green-50 dark:bg-green-900/20 rounded-2xl py-3 mb-3 border border-green-100 dark:border-green-800">
            <span className="text-green-600 dark:text-green-400" style={{ fontSize: "13px", fontWeight: 600 }}>{L.thankRate}</span>
          </div>
        )}
        ...
        <div className="flex items-end gap-3">
          ...
          <button
            ...
            className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 active:scale-90 transition-all ${
              input.trim() ? "bg-amber-500 shadow-md shadow-amber-200" : "bg-slate-200 dark:bg-slate-600"
            }`}
          >
