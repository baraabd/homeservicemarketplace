...
import {
  User, MapPin, Bell, MessageCircle, Settings,
  ChevronRight, ChevronLeft, Star, WifiOff, CheckCircle2,
  BellOff, Wrench, CreditCard,
} from "lucide-react";
...
function NotifIcon({ type }: { type: string }) {
  switch (type) {
    case "bid":       return <Wrench      size={14} className="text-white" />;
    case "tracking":  return <MapPin      size={14} className="text-white" />;
    case "confirmed": return <CheckCircle2 size={14} className="text-white" />;
    case "message":   return <MessageCircle size={14} className="text-white" />;
    case "payment":   return <CreditCard  size={14} className="text-white" />;
...
function NotificationsPage({
  onBack, notifications, onMarkAllRead, onMarkRead,
}: {
  onBack: () => void;
      ...
      <div className="flex-shrink-0 bg-white dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700 shadow-sm">
        <div className="flex items-center gap-3 px-4 py-3.5">
          ...
          <div className="flex items-center gap-2 flex-1">
            ...
            {unread > 0 && (
              <span className="w-5 h-5 rounded-full bg-red-500 text-white flex items-center justify-center" style={{ fontSize: "10px", fontWeight: 800 }}>
                {unread}
              </span>
            )}
          </div>
      ...
      <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
        {notifications.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-slate-400" style={{ fontSize: "14px" }}>{L.empty}</p>
          </div>
        ) : (
          <>
            {notifications.some((n) => !n.read) && (
              <p className="px-5 pt-3 pb-1 text-slate-400" style={{ fontSize: "11px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em" }}>
                {L.new} · {unread}
              </p>
            )}
            {notifications.map((n) => (
              ...
              >
                <div className="relative flex-shrink-0 mt-0.5">
                  <div className={`w-9 h-9 rounded-2xl ${NOTIF_BG[n.type as NotifTypeKey] ?? "bg-slate-500"} flex items-center justify-center shadow-sm`}>
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-slate-900 dark:text-white" style={{ fontSize: "13px", fontWeight: n.read ? 500 : 700, lineHeight: "1.3" }}>
                    {n.title}
                  </p>
                  <p className="text-slate-500 dark:text-slate-400 mt-0.5" style={{ fontSize: "12px", lineHeight: "1.4" }}>
                    {n.body}
                  </p>
                  <p className="text-slate-400 mt-1" style={{ fontSize: "10px" }}>{n.time}</p>
                </div>
                <ChevronRight size={14} className="text-slate-300 rtl:rotate-180 mt-1 flex-shrink-0" />
              </motion.button>
            ))}
            {notifications.some((n) => n.read) && (
              <p className="px-5 pt-3 pb-1 text-slate-400" style={{ fontSize: "11px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em" }}>
...
function MenuItem({
  ...
  return (
    ...
    >
      ...
      <span className="flex-1 text-slate-700 dark:text-slate-200" style={{ fontSize: "14px", fontWeight: 500 }}>
        {label}
      </span>
      {badge !== undefined && badge > 0 && (
        <span className="w-5 h-5 rounded-full bg-red-500 text-white flex items-center justify-center me-1" style={{ fontSize: "10px", fontWeight: 700 }}>
          {badge}
        </span>
      )}
      {dir === "rtl"
...
function ProfileList({
  isOffline, onToggleOffline, unreadCount, onNavigate,
}: {
  isOffline: boolean;
    ...
    <div className="px-4 pt-4 pb-6">
      ...
      <div className="bg-gradient-to-br from-amber-500 to-orange-600 rounded-3xl p-5 mb-4 relative overflow-hidden">
        ...
        <div className="flex items-center gap-4 relative">
          ...
          >
            <span className="text-white" style={{ fontSize: "20px", fontWeight: 800 }}>AK</span>
          </motion.button>
          <div>
            <p className="text-white" style={{ fontSize: "18px", fontWeight: 800 }}>
              {lang === "ar" ? "أحمد الخالد" : "Ahmed Al-Khalid"}
            </p>
            <p className="text-white/70" style={{ fontSize: "13px" }}>ahmed@fixnow.app</p>
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        {[
          ...
          <div key={s.labelKey} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm py-3 flex flex-col items-center">
            <span className="text-slate-900 dark:text-white" style={{ fontSize: "16px", fontWeight: 800 }}>{s.val}</span>
            <span className="text-slate-400" style={{ fontSize: "10px" }}>{t(s.labelKey)}</span>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden mb-4">
        <MenuItem icon={<User          size={16} />} label={t("editProfile")}    onClick={() => onNavigate("editProfile")} />
        <MenuItem icon={<MapPin        size={16} />} label={t("savedAddresses")} onClick={() => onNavigate("savedAddresses")} />
        <MenuItem icon={<Bell          size={16} />} label={t("notifications")}  badge={unreadCount} onClick={() => onNavigate("notifications")} />
        <MenuItem icon={<MessageCircle size={16} />} label={t("helpSupport")}    onClick={() => onNavigate("helpSupport")} />
        <MenuItem icon={<Settings      size={16} />} label={t("settings")}       onClick={() => onNavigate("settings")} />
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${isOffline ? "bg-amber-100" : "bg-green-100"}`}>
              {isOffline
          ...
          >
            <div className={`w-5 h-5 rounded-full bg-white shadow-sm transition-transform duration-300 ${isOffline ? "translate-x-5" : "translate-x-0"}`} />
          </button>
        </div>
      </div>
    </div>
...
// ─── View types ───────────────────────────────────────────────────────────────
export type ProfileView = "editProfile" | "savedAddresses" | "notifications" | "helpSupport" | "settings" | null;
...
export function ProfileTab({
  isOffline, onToggleOffline, notifications, onMarkAllRead, onMarkRead, unreadCount,
}: ProfileTabProps) {
  ...
  return (
    <div className="relative h-full overflow-hidden">
      ...
      <AnimatePresence>
        {view === "editProfile" && (
          <EditProfilePage key="edit" onBack={() => setView(null)} />
        )}
        {view === "savedAddresses" && (
        ...
        {view === "helpSupport" && (
          <HelpSupportPage key="help" onBack={() => setView(null)} />
        )}
        {view === "settings" && (
          <SettingsPage key="settings" onBack={() => setView(null)} />
        )}
      </AnimatePresence>
    </div>
  );
