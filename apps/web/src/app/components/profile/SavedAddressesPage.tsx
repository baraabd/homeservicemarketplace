...
import {
  ChevronLeft, ChevronRight, Plus, Home, Briefcase, MapPin,
  Edit3, Trash2, CheckCircle2, X, Navigation,
} from "lucide-react";
    ...
    setTimeout(() => {
      ...
      if (editId) {
        setAddresses((prev) => prev.map((a) =>
          a.id === editId ? { ...a, label: newLabel, labelAr: newLabel, full: newFull, fullAr: newFull } : a
        ));
      } else {
        setAddresses((prev) => [...prev, {
          id: `a${Date.now()}`, typeKey: "custom",
          label: newLabel, labelAr: newLabel, full: newFull, fullAr: newFull, isDefault: false,
        }]);
      }
      setShowForm(false);
      setEditId(null);
    ...
    >
      ...
      <div className="flex-shrink-0 bg-white dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700 shadow-sm">
        <div className="flex items-center gap-3 px-4 py-3.5">
          ...
          <p className="flex-1 text-slate-900 dark:text-white" style={{ fontSize: "16px", fontWeight: 800 }}>
            {L.title}
          </p>
          <button
            onClick={() => { setEditId(null); setNewLabel(""); setNewFull(""); setShowForm(true); }}
        ...
        {addresses.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <div className="text-center">
              <p className="text-slate-700 dark:text-slate-200" style={{ fontSize: "16px", fontWeight: 700 }}>{L.empty}</p>
              <p className="text-slate-400 mt-1" style={{ fontSize: "13px" }}>{L.emptySub}</p>
            </div>
            <button
              onClick={() => { setEditId(null); setNewLabel(""); setNewFull(""); setShowForm(true); }}
                ...
                >
                  <div className="p-4">
                    <div className="flex items-start gap-3">
                      <div className={`w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0 ${tc.bg}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-0.5">
                          ...
                          {a.isDefault && (
                            <span className="px-2 py-0.5 rounded-full bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400" style={{ fontSize: "10px", fontWeight: 700 }}>
                              {L.defaultBadge}
                            </span>
                          )}
                        </div>
                  ...
                  <AnimatePresence>
                    {delConfirm === a.id && (
                      ...
                      >
                        <p className="text-red-600 dark:text-red-400" style={{ fontSize: "12px", fontWeight: 600 }}>
                          {L.deleteQ}
                        </p>
          ...
          <>
            ...
            >
              ...
              <div className="flex flex-col gap-3 mb-4">
                ...
                />
                <button
                  onClick={() => setNewFull(lang === "ar" ? "الموقع الحالي (تم التحديد)" : "Current Location (detected)")}
                ...
                >
                  ...
                  <span className="text-blue-700 dark:text-blue-400" style={{ fontSize: "13px", fontWeight: 600 }}>{L.getLocation}</span>
                </button>
              </div>

              <Button
                variant="primary"
                state={saving ? "loading" : !newLabel.trim() || !newFull.trim() ? "disabled" : "default"}
                fullWidth
                onClick={handleSave}
              >
                {L.save}
