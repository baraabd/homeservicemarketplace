...
import {
  ChevronLeft, ChevronRight, Mail, Lock, Eye, EyeOff, User, Phone,
  AlertCircle, CheckCircle2, Send, ArrowRight, ArrowLeft,
} from "lucide-react";
    ...
    <div className="flex flex-col bg-white" style={{ minHeight: "100svh" }}>
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-amber-500 via-amber-500 to-orange-600 px-6 pt-16 pb-20">
        ...
        >
          ...
          <span className="text-white/90" style={{ fontSize: "12px", fontWeight: 600 }}>Apps</span>
        </button>
        ...
        <div className="relative flex flex-col items-center text-center">
          ...
          <h1 className="text-white" style={{ fontSize: "28px", fontWeight: 800 }}>{t("appName")}</h1>
          <p className="text-white/70 mt-1" style={{ fontSize: "13px" }}>{t("appTagline")}</p>
        </div>
      </div>
      ...
      <div className="flex-1 bg-white rounded-t-3xl -mt-6 px-6 pt-8 pb-6">
        ...
        <div className="flex flex-col gap-4">
          ...
          <TextField
            ...
            trailingIcon={
              <button type="button" onClick={() => setShowPw((v) => !v)} className="flex items-center justify-center w-5 h-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-slate-400" style={{ fontSize: "11px" }}>{t("orContinueWith")}</span>
            <div className="flex-1 h-px bg-slate-200" />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <button className="h-12 rounded-2xl border-2 border-slate-200 flex items-center justify-center gap-2 text-slate-700 active:bg-slate-50 transition-all" style={{ fontSize: "13px", fontWeight: 600 }}>
          ...
          <p className="text-center text-slate-500 pb-2" style={{ fontSize: "14px" }}>
            {t("noAccount")}{" "}
            <button onClick={onSignUp} className="text-amber-600 active:opacity-70" style={{ fontWeight: 700 }}>
              {t("signUp")}
            </button>
          </p>
        </div>
  ...
  return (
    <SwipeableScreen onBack={step > 1 ? () => setStep((s) => s - 1) : onBack}>
      <div className="flex flex-col bg-white" style={{ height: "100svh" }}>
        <AuthTopBar title={t("createAccount")} onBack={step > 1 ? () => setStep((s) => s - 1) : onBack} />
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6" style={{ scrollbarWidth: "none" }}>
              ...
              <div key={label} className="flex items-center gap-2 flex-1">
                <div
                  className={`flex items-center justify-center w-7 h-7 rounded-full flex-shrink-0 transition-all ${
                    ...
                    i + 1 === step ? "bg-amber-500 text-white ring-2 ring-amber-200" :
                ...
                <span style={{ fontSize: "10px", fontWeight: i + 1 <= step ? 700 : 400, color: i + 1 <= step ? "#F59E0B" : "#94a3b8" }}>
                  {label}
                </span>
                {i < 2 && <div className={`flex-1 h-0.5 rounded-full ${i + 1 < step ? "bg-amber-300" : "bg-slate-100"}`} />}
              </div>
            ))}
          </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-slate-50 rounded-2xl border border-slate-200 px-4 py-3 flex items-center gap-3">
                <div className="w-8 h-8 rounded-xl bg-amber-100 flex items-center justify-center flex-shrink-0">
                  <span className="text-amber-600" style={{ fontSize: "16px" }}>📍</span>
                </div>
                <div>
                  <p className="text-slate-700" style={{ fontSize: "13px", fontWeight: 600 }}>Riyadh, Saudi Arabia</p>
                  <p className="text-slate-400" style={{ fontSize: "11px" }}>Auto-detected location</p>
                </div>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="flex flex-col items-center text-center gap-4">
              ...
              <div>
                <h3 className="text-slate-900" style={{ fontSize: "18px", fontWeight: 800 }}>Verify Your Number</h3>
                ...
                {[0, 1, 2, 3].map((i) => (
                  <div
                    ...
                    className={`w-14 h-14 rounded-2xl border-2 flex items-center justify-center transition-all ${
                      otp.length > i ? "border-amber-500 bg-amber-50" : "border-slate-200 bg-slate-50"
                    }`}
        ...
        <div className="bg-white border-t border-slate-100 px-6 py-4 shadow-[0_-4px_20px_rgba(0,0,0,0.06)]">
          ...
          <p className="text-center text-slate-400 mt-3" style={{ fontSize: "12px" }}>
            ...
            <button onClick={onBack} className="text-amber-600 font-semibold">{t("login")}</button>
          </p>
        </div>
      </div>
    </SwipeableScreen>
        ...
        <div className="flex-1 px-6 py-8">
          {submitted ? (
            <div className="flex flex-col items-center text-center py-8">
              ...
              <p className="text-slate-400 mb-2 max-w-[260px]" style={{ fontSize: "14px", lineHeight: "1.6" }}>
                {t("sentResetTo")}
              </p>
              <div className="inline-flex items-center gap-2 bg-amber-50 border border-amber-100 rounded-2xl px-4 py-2 mb-8">
                <Mail size={14} className="text-amber-600" />
                <span className="text-amber-700" style={{ fontSize: "14px", fontWeight: 600 }}>{email}</span>
              </div>
