...
import {
  X, ChevronLeft, ChevronRight, Camera, Upload, MapPin, Calendar, Clock,
  CheckCircle2, Wrench, Wind, Hammer, Sparkles, WifiOff,
  Image as ImageIcon, FileText, ArrowRight, Zap,
  ChevronDown, Maximize2, PaintBucket,
} from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute bg-white/80" style={{ top: "35%", left: 0, right: 0, height: "14px" }} />
        <div className="absolute bg-white/60" style={{ top: "68%", left: 0, right: 0, height: "8px" }} />
        <div className="absolute bg-white/80" style={{ left: "28%", top: 0, bottom: 0, width: "14px" }} />
        <div className="absolute bg-white/60" style={{ left: "65%", top: 0, bottom: 0, width: "8px" }} />
        {[
      ...
      <div className="absolute top-3 inset-x-0 flex justify-center">
        <div className="flex items-center gap-2 bg-white/90 backdrop-blur rounded-xl px-3 py-1.5 shadow-sm">
          ...
          <span className="text-slate-600" style={{ fontSize: "11px", fontWeight: 600 }}>{label}</span>
        </div>
      </div>
    </div>
  );
          ...
          <div key={label} className="flex items-center gap-1.5">
            <div
              className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 transition-all ${
                i + 1 < current  ? "bg-amber-500" :
                i + 1 === current ? "bg-amber-500 ring-2 ring-amber-200" :
                                    "bg-slate-200"
              }`}
            >
              {i + 1 < current ? (
                ...
                <span className={i + 1 === current ? "text-white" : "text-slate-400"} style={{ fontSize: "9px", fontWeight: 800 }}>
                  {i + 1}
                </span>
              )}
            </div>
...
export function JobWizardModal({ service, isOpen, onClose, isOffline }: JobWizardModalProps) {
  ...
  const [address,   setAddress]   = useState(t("address") === "Address" ? "Al Olaya District, Riyadh" : "حي العليا، الرياض");
  const [schedule,  setSchedule]  = useState<"asap" | "later">("asap");
  const [uploads,   setUploads]   = useState<string[]>([]);
  const [isPosting, setIsPosting] = useState(false);
  const [expanded,  setExpanded]  = useState(false);

  const { onTouchStart: handleTouchStart, onTouchMove: handleTouchMove, onTouchEnd: handleTouchEnd, dragY } = useSwipe({
  ...
  const handlePost = () => {
    setIsPosting(true);
    setTimeout(() => { setIsPosting(false); setStep(3); }, 2000);
  };
    ...
    >
      ...
      >
        {step < 3 && (
          ...
          >
            ...
            <div className="flex items-center gap-1 text-slate-400 pb-1">
              {expanded
                ? <><ChevronDown size={12} /><span style={{ fontSize: "10px" }}>{t("swipeDown")}</span><ChevronDown size={12} /></>
                : <><Maximize2 size={10} /><span style={{ fontSize: "10px" }}>{t("swipeUp")}</span></>
              }
            </div>
          </div>
        )}
          ...
          <>
            <div className="flex items-center justify-between px-5 pt-4 pb-3 flex-shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-slate-900" style={{ fontSize: "16px", fontWeight: 800 }}>{t("postJob")}</p>
                  <p className="text-slate-400" style={{ fontSize: "12px" }}>{service}</p>
                </div>
              </div>
              <button onClick={handleClose} className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center active:scale-90">
            ...
            <div className="flex-1 overflow-y-auto px-5" style={{ scrollbarWidth: "none" }}>
              {/* Upload zone */}
              <p className="text-slate-700 mb-2.5" style={{ fontSize: "13px", fontWeight: 600 }}>{t("uploadPhotos")}</p>
              <button
              ...
              >
                ...
                <div>
                  <p className="text-slate-700" style={{ fontSize: "13px", fontWeight: 600 }}>{t("tapToUpload")}</p>
                  <p className="text-slate-400 mt-0.5" style={{ fontSize: "11px" }}>{t("uploadTypes")}</p>
                </div>
              ...
              {uploads.length > 0 && (
                <div className="flex gap-2 mb-4 flex-wrap">
                  {uploads.map((color, i) => (
                    <div key={i} className={`w-16 h-16 rounded-xl ${color} flex items-center justify-center relative`}>
                  ...
                  {uploads.length < 4 && (
                    ...
                    >
                      <span className="text-slate-400" style={{ fontSize: "22px", lineHeight: 1 }}>+</span>
                    </button>
                  )}
                </div>
              )}
              ...
              <div className="mb-4">
                <p className="text-slate-700 mb-2.5" style={{ fontSize: "13px", fontWeight: 600 }}>{t("describeTheProblem")}</p>
                <TextField
            ...
            <div className="flex-shrink-0 border-t border-slate-100 px-5 py-4 bg-white">
              <Button variant="primary" fullWidth onClick={() => setStep(2)} leadingIcon={<ArrowRight size={16} />}>
                {t("nextStep")}
              </Button>
            </div>
          </>
        ...
        {step === 2 && (
          <>
            <div className="flex items-center justify-between px-5 pt-4 pb-3 flex-shrink-0">
              <button onClick={() => setStep(1)} className="w-9 h-9 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center active:scale-90">
                <BackChevron />
              </button>
              <p className="text-slate-900" style={{ fontSize: "16px", fontWeight: 800 }}>{t("locationAndTime")}</p>
              <button onClick={handleClose} className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center active:scale-90">
                <X size={16} className="text-slate-600" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-5" style={{ scrollbarWidth: "none" }}>
              <p className="text-slate-700 mb-2.5" style={{ fontSize: "13px", fontWeight: 600 }}>{t("serviceLocation")}</p>
              ...
              <p className="text-slate-700 mb-2.5" style={{ fontSize: "13px", fontWeight: 600 }}>{t("whenDoYouNeed")}</p>
                ...
                {(["asap", "later"] as const).map((opt) => (
                  <button
                    ...
                    className={`flex-1 py-3 rounded-2xl border-2 flex items-center justify-center gap-2 transition-all active:scale-95 ${
                      schedule === opt ? "border-amber-500 bg-amber-50" : "border-slate-200 bg-white"
                    }`}
                  >
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${schedule === opt ? "border-amber-500" : "border-slate-300"}`}>
                      {schedule === opt && <div className="w-2 h-2 rounded-full bg-amber-500" />}
                    </div>
                    {opt === "asap"
                      ? <Clock    size={14} className={schedule === opt ? "text-amber-600" : "text-slate-400"} />
                      : <Calendar size={14} className={schedule === opt ? "text-amber-600" : "text-slate-400"} />
                    }
                    <span style={{ fontSize: "13px", fontWeight: 700 }} className={schedule === opt ? "text-amber-700" : "text-slate-500"}>
              ...
              {schedule === "later" && (
                <div className="bg-slate-50 rounded-2xl border border-slate-200 p-4 mb-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <p className="text-slate-500 mb-2" style={{ fontSize: "11px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em" }}>
                        {t("date")}
                      </p>
                      <div className="flex items-center gap-2 bg-white rounded-xl border border-slate-200 px-3 py-2.5">
                        <Calendar size={14} className="text-amber-500" />
                        <span className="text-slate-700" style={{ fontSize: "13px", fontWeight: 500 }}>{t("dateValue")}</span>
                      </div>
                    </div>
                    <div>
                      <p className="text-slate-500 mb-2" style={{ fontSize: "11px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em" }}>
                      ...
                      <div className="flex items-center gap-2 bg-white rounded-xl border border-slate-200 px-3 py-2.5">
                        ...
                        <span className="text-slate-700" style={{ fontSize: "13px", fontWeight: 500 }}>{t("timeValue")}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {schedule === "asap" && (
                <div className="flex items-start gap-2 bg-green-50 border border-green-200 rounded-2xl px-4 py-3 mb-4">
                  ...
                  <p className="text-green-700" style={{ fontSize: "12px", lineHeight: "1.5" }}>{t("asapContext")}</p>
                </div>
              )}
              <div className="h-2" />
            </div>
          ...
          <div className="flex-1 flex flex-col bg-white" style={{ minHeight: "100svh" }}>
            {isOffline && (
              <div className="flex items-center gap-3 bg-slate-900 px-5 py-3.5">
                ...
                <p className="flex-1 text-white" style={{ fontSize: "12px", fontWeight: 500, lineHeight: "1.4" }}>
                  {t("savedLocally")}
                </p>
                <button onClick={handleClose} className="w-6 h-6 rounded-full bg-white/10 flex items-center justify-center">
            ...
            <div className="flex-1 flex flex-col items-center justify-center px-6 py-8 text-center">
              ...
              <div className="relative mb-6">
                ...
                {[
                  ...
                  <div key={i} className={`absolute rounded-full ${dot.color} ${dot.size}`} style={{ top: dot.top, left: dot.left }} />
                ))}
              </div>
              ...
              <p className="text-slate-400 mb-8 max-w-[260px]" style={{ fontSize: "14px", lineHeight: "1.7" }}>
                {t("jobPosted").replace("! 🎉", "")} · {service}
              </p>

              {/* Summary */}
              <div className="w-full bg-slate-50 rounded-3xl border border-slate-200 p-4 mb-8 text-start">
                <p className="text-slate-500 mb-3" style={{ fontSize: "11px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em" }}>
                  {t("jobSummary")}
                </p>
                {[
                  { label: t("serviceLabel"),  val: service                                                      },
                  { label: t("locationLabel"), val: address                                                       },
                  { label: t("scheduleLabel"), val: schedule === "asap" ? t("asapFull") : `${t("dateValue")} ${t("timeValue")}` },
                  { label: t("statusLabel"),   val: isOffline ? t("savedLocally") : t("postedAndLive")            },
                ].map((row) => (
                  <div key={row.label} className="flex justify-between py-2 border-b border-slate-100 last:border-0">
                    <span className="text-slate-400" style={{ fontSize: "12px" }}>{row.label}</span>
                    <span className="text-slate-900" style={{ fontSize: "12px", fontWeight: 600 }}>{row.val}</span>
                  </div>
                ))}
              </div>

              <div className="w-full flex flex-col gap-3">
                <Button variant="primary"   fullWidth onClick={handleClose}>{t("backToHome")}</Button>
                <Button variant="secondary" fullWidth onClick={handleClose}>{t("viewMyJobs")}</Button>
              </div>
