...
import { useEcosystem, WALLET_TRANSACTIONS, EARNINGS_CHART_DATA } from "../../context/EcosystemContext";
import type { ServiceRequest } from "../../context/EcosystemContext";
import { ImageWithFallback } from "../ui/ImageWithFallback";

// ─── Map image (unsplash) ────────────────────────────────────────────────────
const MAP_IMG = "https://images.unsplash.com/photo-1554616242-a3e806a99481?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxjaXR5JTIwYWVyaWFsJTIwbWFwJTIwc3RyZWV0cyUyMHNhdGVsbGl0ZSUyMHZpZXd8ZW58MXx8fHwxNzczMjQ4NTc5fDA&ixlib=rb-4.1.0&q=80&w=1080";
...
function BiddingModal({
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      ...
      setSending(false);
      setDone(true);
      setTimeout(() => { onClose(); setDone(false); }, 1600);
    }, 1200);
  };
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 mb-4">
          <div>
            <p className="text-slate-900 dark:text-white" style={{ fontSize: "18px", fontWeight: 800 }}>{L.title}</p>
            <p className="text-slate-400" style={{ fontSize: "12px" }}>
              {L.requestFor} <span className="text-amber-600 font-semibold">{lang === "ar" ? request.serviceAr : request.service}</span>
            </p>
          </div>
          <button onClick={onClose} className="w-9 h-9 rounded-xl bg-slate-100 dark:bg-slate-700 flex items-center justify-center active:scale-90">
        ...
        {done ? (
          ...
          >
            ...
            <p className="text-slate-900 dark:text-white" style={{ fontSize: "16px", fontWeight: 700 }}>{L.sent}</p>
          </motion.div>
        ) : (
          <div className="px-5 pb-6 flex flex-col gap-4">
            {/* Price */}
            <div>
              <p className="text-slate-600 dark:text-slate-300 mb-2" style={{ fontSize: "13px", fontWeight: 600 }}>{L.priceLabel}</p>
              <div className="flex items-center gap-3 bg-slate-100 dark:bg-slate-700 rounded-2xl px-4 py-3">
                ...
                />
                <span className="text-slate-400" style={{ fontSize: "12px" }}>USD/hr</span>
              </div>
            </div>

            {/* Time chips */}
            <div>
              <p className="text-slate-600 dark:text-slate-300 mb-2" style={{ fontSize: "13px", fontWeight: 600 }}>{L.timeLabel}</p>
            ...
            {/* Note */}
            <div>
              <p className="text-slate-600 dark:text-slate-300 mb-2" style={{ fontSize: "13px", fontWeight: 600 }}>{L.noteLabel}</p>
              <textarea
                value={note}
            ...
            >
              {sending ? (
                <><span className="w-4 h-4 rounded-full border-2 border-white/30 border-t-white animate-spin" />{L.sending}</>
              ) : (
...
// ─── Job pin component ────────────────────────────────────────────────────────
function JobPin({ req, onTap, isSelected }: { req: ServiceRequest; onTap: () => void; isSelected: boolean }) {
  const urgentColor = req.urgency === "urgent" ? "#ef4444" : "#3b82f6";
  const statusColor = req.status === "pending" ? urgentColor : req.status === "bidding" ? "#f59e0b" : "#10b981";
  return (
    <div
      className="absolute cursor-pointer"
      style={{ left: `${req.mapX}%`, top: `${req.mapY}%`, transform: "translate(-50%,-50%)", zIndex: isSelected ? 20 : 10 }}
    ...
    >
      ...
      {isSelected && (
        ...
        >
          <p className="text-slate-900 dark:text-white" style={{ fontSize: "13px", fontWeight: 700 }}>{req.service}</p>
          <p className="text-slate-400" style={{ fontSize: "11px" }}>{req.distance}km · {req.budget}</p>
          <div className="flex items-center gap-1.5 mt-1">
            ...
            <span style={{ fontSize: "10px", color: statusColor, fontWeight: 600 }}>
              {req.status === "pending" ? "Awaiting bids" : req.status === "bidding" ? `${req.bids.length} bids` : "Assigned"}
            </span>
          </div>
        </motion.div>
      )}
...
function LiveJobsScreen() {
  ...
  const activeReqs = requests.filter((r) => r.status !== "completed" && r.status !== "assigned" || r.status === "assigned");
  const filteredReqs = filterStatus === "all" ? activeReqs : activeReqs.filter((r) => r.status === filterStatus);
  ...
  const handleBidSubmit = (price: number, time: string, note: string) => {
    ...
    submitBid(biddingReq.id, {
      providerName: "Omar K.", providerAr: "عمر خ.", providerRating: 4.8,
      providerJobs: 156, price, executionTime: time, note,
    });
    setBiddingReq(null);
  };
      ...
      >
        ...
        <div className="absolute top-4 start-4 end-4 flex items-center justify-between z-10">
          <div className="flex items-center gap-2 bg-black/60 backdrop-blur-md rounded-2xl px-3 py-2 border border-white/10">
            ...
            <span className="text-white" style={{ fontSize: "12px", fontWeight: 600 }}>{L.online}</span>
          </div>
          <div className="flex items-center gap-2 bg-black/60 backdrop-blur-md rounded-2xl px-3 py-2 border border-white/10">
            <Navigation size={12} className="text-blue-400" />
            <span className="text-white" style={{ fontSize: "12px" }}>Riyadh, SA</span>
          </div>
        </div>
        ...
        {!sheetOpen && (
          ...
          >
            <div className="bg-white/90 backdrop-blur-md rounded-full px-4 py-2 shadow-lg border border-white/20 flex items-center gap-2">
              ...
              <span className="text-slate-700" style={{ fontSize: "12px", fontWeight: 600 }}>{L.drag}</span>
        ...
        {sheetOpen && (
          ...
          >
            ...
            <div className="flex-shrink-0 flex flex-col items-center pt-3 pb-2">
              <button onClick={() => setSheetOpen(false)} className="w-10 h-1 rounded-full bg-slate-200 dark:bg-slate-600 mb-2" />
              <div className="flex items-center justify-between w-full px-4 pb-2">
                <p className="text-slate-900 dark:text-white" style={{ fontSize: "16px", fontWeight: 800 }}>
                  ...
                  {(["all", "pending", "bidding"] as const).map((f) => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-xl transition-all ${
                        filterStatus === f ? "bg-blue-600 text-white" : "bg-slate-100 dark:bg-slate-700 text-slate-500"
                      }`}
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-4" style={{ scrollbarWidth: "none" }}>
              {filteredReqs.length === 0 ? (
                <div className="flex flex-col items-center py-10 gap-3">
                  ...
                  <p className="text-slate-400" style={{ fontSize: "14px" }}>{L.noJobs}</p>
                </div>
              ) : (
                filteredReqs.map((req) => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-0.5">
                          ...
                          {req.urgency === "urgent" && (
                            <span className="px-1.5 py-0.5 rounded-lg bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400" style={{ fontSize: "9px", fontWeight: 700 }}>
                              {L.urgentTag}
                            </span>
                          )}
                        ...
                        <p className="text-slate-400 dark:text-slate-400" style={{ fontSize: "12px" }}>
                          {lang === "ar" ? req.locationAr : req.location}
                        </p>
                        <div className="flex items-center gap-3 mt-1.5">
                          <div className="flex items-center gap-1">
                            <MapPin size={11} className="text-blue-500" />
                            <span className="text-blue-600 dark:text-blue-400" style={{ fontSize: "11px", fontWeight: 600 }}>{req.distance}{L.km}</span>
                          </div>
                          <div className="flex items-center gap-1">
                            <DollarSign size={11} className="text-green-500" />
                            <span className="text-green-600 dark:text-green-400" style={{ fontSize: "11px", fontWeight: 600 }}>{req.budget}</span>
                          </div>
                          <div className="flex items-center gap-1">
                            <Star size={11} className="text-amber-500 fill-amber-500" />
                            <span className="text-slate-500" style={{ fontSize: "11px" }}>{req.seekerName}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    {req.status !== "assigned" && req.status !== "completed" && (
                      <motion.button
                        whileTap={{ scale: 0.95 }}
                        onClick={(e) => { e.stopPropagation(); setBiddingReq(req); setSheetOpen(false); }}
...
function MyBidsScreen() {
  ...
  const myBids = requests.flatMap((req) =>
    req.bids
      ...
      .map((b) => ({ ...b, requestService: req.service, requestServiceAr: req.serviceAr, requestIcon: req.serviceIcon, seekerName: req.seekerName }))
  );
  ...
  const L = {
    ...
    noBidsSub: lang === "ar" ? "ابدأ بتقديم عروض على الطلبات القريبة" : "Start placing bids on nearby requests",
    for:       lang === "ar" ? "من"                  : "for",
  };

  const STATUS_STYLE: Record<string, { bg: string; text: string; label: string }> = {
    pending:  { bg:"bg-amber-100 dark:bg-amber-900/30", text:"text-amber-700 dark:text-amber-400", label:L.pending  },
    accepted: { bg:"bg-green-100 dark:bg-green-900/30", text:"text-green-700 dark:text-green-400", label:L.accepted },
    rejected: { bg:"bg-red-100 dark:bg-red-900/30",     text:"text-red-600 dark:text-red-400",     label:L.rejected },
  };

  return (
    <div className="absolute inset-0 flex flex-col bg-slate-50 dark:bg-slate-900 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
      {/* Header */}
      <div className="flex-shrink-0 bg-white dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700 px-5 pt-5 pb-4">
        <h2 className="text-slate-900 dark:text-white" style={{ fontSize: "22px", fontWeight: 800 }}>{L.title}</h2>
        <div className="flex gap-2 mt-3">
          {(["pending","accepted","rejected"] as const).map((s) => {
            const cnt = myBids.filter((b) => b.status === s).length;
            return (
              <div key={s} className={`px-3 py-1.5 rounded-xl flex items-center gap-1.5 ${STATUS_STYLE[s].bg}`}>
                <span className={STATUS_STYLE[s].text} style={{ fontSize: "11px", fontWeight: 700 }}>{STATUS_STYLE[s].label}</span>
                <span className={`w-4 h-4 rounded-full flex items-center justify-center ${STATUS_STYLE[s].text}`} style={{ fontSize: "9px", fontWeight: 800, background:"rgba(0,0,0,0.08)" }}>
                  {cnt}
                </span>
              </div>
            );
        ...
        {myBids.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <div className="text-center">
              <p className="text-slate-700 dark:text-white" style={{ fontSize: "16px", fontWeight: 700 }}>{L.noBids}</p>
              <p className="text-slate-400 mt-1" style={{ fontSize: "13px" }}>{L.noBidsSub}</p>
            </div>
          </div>
        ) : (
          myBids.map((bid) => {
                ...
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`px-2 py-0.5 rounded-lg ${ss.bg} ${ss.text}`} style={{ fontSize: "10px", fontWeight: 700 }}>
                        {ss.label}
                      </span>
                    </div>
                    <p className="text-slate-400" style={{ fontSize: "12px" }}>{L.for} {bid.seekerName} · {bid.submittedAt}</p>
                  </div>
                </div>

                <div className="flex gap-3 mb-3">
                  <div className="flex-1 bg-slate-50 dark:bg-slate-700 rounded-2xl px-3 py-2.5">
                    <p className="text-slate-400" style={{ fontSize: "10px" }}>{L.price}</p>
                    <p className="text-slate-900 dark:text-white" style={{ fontSize: "16px", fontWeight: 800 }}>${bid.price}/hr</p>
                  </div>
                  <div className="flex-1 bg-slate-50 dark:bg-slate-700 rounded-2xl px-3 py-2.5">
                    <p className="text-slate-400" style={{ fontSize: "10px" }}>{L.time}</p>
                    <p className="text-slate-900 dark:text-white" style={{ fontSize: "13px", fontWeight: 700 }}>{bid.executionTime}</p>
                  </div>
                </div>

                {bid.note && (
                  <div className="bg-slate-50 dark:bg-slate-700 rounded-2xl px-3 py-2 mb-3">
                    <p className="text-slate-500 dark:text-slate-400" style={{ fontSize: "12px", lineHeight: "1.4" }}>"{bid.note}"</p>
                  </div>
                )}

                {bid.status === "accepted" && (
                  <button className="w-full py-3 rounded-2xl bg-green-600 text-white flex items-center justify-center gap-2 active:scale-95 transition-all shadow-md shadow-green-200 dark:shadow-none" style={{ fontSize: "14px", fontWeight: 700 }}>
    ...
    <div className="absolute inset-0 flex flex-col bg-slate-50 dark:bg-slate-900 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
      ...
      <div className="bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-700 mx-4 mt-4 rounded-3xl p-6 relative overflow-hidden">
        ...
        <div className="relative">
          <p className="text-white/70 mb-1" style={{ fontSize: "12px" }}>{L.balance}</p>
          <p className="text-white" style={{ fontSize: "38px", fontWeight: 900, letterSpacing: "-0.02em" }}>$1,240.00</p>
          <div className="flex gap-4 mt-4">
            <div className="bg-white/15 rounded-2xl px-3 py-2">
              <p className="text-white/60" style={{ fontSize: "10px" }}>{L.pending}</p>
              <p className="text-white" style={{ fontSize: "16px", fontWeight: 700 }}>$45.00</p>
            </div>
            <div className="bg-white/15 rounded-2xl px-3 py-2">
              <p className="text-white/60" style={{ fontSize: "10px" }}>{L.thisWeek}</p>
              <p className="text-white" style={{ fontSize: "16px", fontWeight: 700 }}>$310.00</p>
            </div>
            <div className="bg-white/15 rounded-2xl px-3 py-2">
              <p className="text-white/60" style={{ fontSize: "10px" }}>{L.thisMonth}</p>
              <p className="text-white" style={{ fontSize: "16px", fontWeight: 700 }}>$1,240</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 mt-3">
        <button
          ...
          className={`w-full py-4 rounded-2xl flex items-center justify-center gap-2.5 transition-all active:scale-95 ${
            withdrawn ? "bg-green-600 text-white" : "bg-blue-600 text-white shadow-lg shadow-blue-200 dark:shadow-none"
          }`}
          style={{ fontSize: "15px", fontWeight: 800 }}
        >
          {withdrawing ? (
            <><span className="w-4 h-4 rounded-full border-2 border-white/30 border-t-white animate-spin" />{L.withdrwing}</>
          ) : withdrawn ? (
            <><CheckCircle2 size={18} />{L.withdrawn}</>
          ) : (
            <><TrendingUp size={18} />{L.withdraw}</>
          )}
        </button>
      </div>
      ...
      <div className="mx-4 mt-4 bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm p-4">
        <p className="text-slate-900 dark:text-white mb-3" style={{ fontSize: "14px", fontWeight: 700 }}>{L.weeklyEarn}</p>
        <ResponsiveContainer width="100%" height={110}>
          <AreaChart data={EARNINGS_CHART_DATA} margin={{ top: 5, right: 5, bottom: 0, left: -25 }}>
            ...
            />
            <Area type="monotone" dataKey="earn" stroke="#3b82f6" strokeWidth={2.5} fill="url(#blueGrad)" dot={{ fill: "#3b82f6", r: 3 }} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="mx-4 mt-4 mb-4">
        <p className="text-slate-900 dark:text-white mb-3" style={{ fontSize: "15px", fontWeight: 700 }}>{L.history}</p>
        <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          {WALLET_TRANSACTIONS.map((tx, i) => (
            <div key={tx.id} className={`flex items-center gap-3 px-4 py-3.5 ${i > 0 ? "border-t border-slate-50 dark:border-slate-700" : ""}`}>
              <div className={`w-9 h-9 rounded-2xl flex items-center justify-center flex-shrink-0 ${
                tx.type === "earning" ? "bg-green-100 dark:bg-green-900/30" :
                tx.type === "pending" ? "bg-amber-100 dark:bg-amber-900/30" :
                "bg-red-100 dark:bg-red-900/30"
              }`}>
                {tx.type === "earning" ? <TrendingUp size={14} className="text-green-600" /> :
                 tx.type === "pending" ? <Clock       size={14} className="text-amber-600" /> :
                                        <ArrowLeft    size={14} className="text-red-600"   />}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-slate-800 dark:text-slate-100" style={{ fontSize: "13px", fontWeight: 600 }}>
      ...
      <div className="bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-700 px-5 pt-6 pb-8 relative overflow-hidden">
        ...
        <div className="flex items-center gap-4 relative">
          <div className="w-20 h-20 rounded-3xl bg-white/20 border-2 border-white/30 flex items-center justify-center">
            <span className="text-white" style={{ fontSize: "24px", fontWeight: 900 }}>OK</span>
          </div>
          <div>
            ...
            <div className="flex items-center gap-1 mt-0.5">
              ...
              <span className="text-white/70" style={{ fontSize: "12px" }}>{L.level}</span>
            </div>
            <p className="text-white/50 mt-0.5" style={{ fontSize: "11px" }}>{L.memberSince} Jan 2023</p>
          </div>
        </div>
        {/* Stats */}
        <div className="grid grid-cols-3 gap-2 mt-5 relative">
          {[
            ...
            <div key={s.label} className="bg-white/15 rounded-2xl py-3 flex flex-col items-center">
              <span className="text-white" style={{ fontSize: "15px", fontWeight: 800 }}>{s.val}</span>
              <span className="text-white/60" style={{ fontSize: "10px" }}>{s.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="px-4 -mt-3 z-10 relative">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm p-4 mb-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${available ? "bg-green-100" : "bg-slate-100"}`}>
              {available ? <CheckCircle2 size={16} className="text-green-600" /> : <WifiOff size={16} className="text-slate-500" />}
            </div>
            <div>
              <p className="text-slate-900 dark:text-white" style={{ fontSize: "14px", fontWeight: 700 }}>
              ...
              <p className="text-slate-400" style={{ fontSize: "11px" }}>
                {available ? (lang === "ar" ? "تستقبل الطلبات" : "Receiving requests") : (lang === "ar" ? "مخفي عن الخريطة" : "Hidden from map")}
              </p>
            </div>
          </div>
          <button
          ...
          >
            <div className={`w-5 h-5 rounded-full bg-white shadow-sm transition-transform duration-300 ${available ? "translate-x-5" : "translate-x-0"}`} />
          </button>
        </div>

        {/* Skills */}
        <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm p-4 mb-4">
          <p className="text-slate-900 dark:text-white mb-3" style={{ fontSize: "15px", fontWeight: 700 }}>{L.skills}</p>
          <div className="flex flex-wrap gap-2">
            {SKILLS.map((sk) => (
              <span key={sk.en} className={`px-3 py-1.5 rounded-xl ${sk.color} dark:bg-slate-700 dark:text-slate-200`} style={{ fontSize: "13px", fontWeight: 600 }}>
                {lang === "ar" ? sk.ar : sk.en}
              </span>
            ))}
          </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden mb-4">
          {[
            ...
            <button key={i} className={`w-full flex items-center gap-3 px-4 py-3.5 active:bg-slate-50 dark:active:bg-slate-700/50 transition-all text-start ${i > 0 ? "border-t border-slate-50 dark:border-slate-700" : ""}`}>
              ...
              <span className="flex-1 text-slate-700 dark:text-slate-200" style={{ fontSize: "14px", fontWeight: 500 }}>{label}</span>
              <ChevronRight size={16} className="text-slate-300 dark:text-slate-600 rtl:rotate-180" />
            </button>
          ))}
        </div>
...
export function ProviderApp() {
  ...
  const renderTab = () => {
    switch (activeTab) {
      case "jobs":    return <LiveJobsScreen />;
      case "bids":    return <MyBidsScreen />;
      case "wallet":  return <WalletScreen />;
      case "profile": return <ProviderProfileScreen />;
      default:        return <LiveJobsScreen />;
    }
  };
          ...
          <div className="flex items-center justify-between px-5 py-3.5">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-700 flex items-center justify-center shadow-sm">
                <span className="text-white" style={{ fontSize: "12px", fontWeight: 800 }}>OK</span>
              </div>
              <div>
                <p className="text-slate-400" style={{ fontSize: "11px" }}>{lang === "ar" ? "مرحباً 👋" : "Welcome back 👋"}</p>
                <p className="text-slate-900 dark:text-white" style={{ fontSize: "14px", fontWeight: 700 }}>
                  {lang === "ar" ? "عمر الخالد" : "Omar Al-Khalid"}
                </p>
              </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button className="relative w-9 h-9 rounded-xl bg-slate-50 dark:bg-slate-700 border border-slate-100 dark:border-slate-600 flex items-center justify-center active:scale-90 transition-all">
                ...
                <span className="absolute -top-1 -end-1 w-4 h-4 rounded-full bg-red-500 text-white flex items-center justify-center border-2 border-white" style={{ fontSize: "8px", fontWeight: 800 }}>2</span>
              </button>
            </div>
          </div>
        </div>
          ...
          {PROVIDER_NAV.map(({ id, icon: Icon, labelEn, labelAr }) => {
            ...
            return (
              ...
              >
                ...
                <Icon size={22} className={`relative z-10 transition-colors ${active ? "text-blue-600" : "text-slate-400"}`} />
                <span className="relative z-10" style={{ fontSize: "10px", fontWeight: active ? 700 : 500, color: active ? "#2563eb" : "#94a3b8" }}>
