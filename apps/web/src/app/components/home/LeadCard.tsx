...
const statusConfig: Record<LeadStatus, { label: string; dot: string; badge: string; text: string }> = {
  pending:   { label: "Pending Bids", dot: "bg-amber-500",  badge: "bg-amber-50  border-amber-200",  text: "text-amber-700"  },
  active:    { label: "Pro Assigned", dot: "bg-blue-500",   badge: "bg-blue-50   border-blue-200",   text: "text-blue-700"   },
  completed: { label: "Completed",    dot: "bg-green-500",  badge: "bg-green-50  border-green-200",  text: "text-green-700"  },
  cancelled: { label: "Cancelled",    dot: "bg-red-400",    badge: "bg-red-50    border-red-200",    text: "text-red-600"    },
};

export function LeadCard({ service, status, proName, proInitials, postedAt, price, bids, onClick, showPrice = true }: LeadCardProps) {
    ...
    >
      ...
      <div
        className={`h-1 w-full rounded-t-3xl ${
          ...
          status === "active"    ? "bg-gradient-to-r from-blue-500 to-indigo-500"  :
      ...
      <div className="p-4">
        ...
        {/* Status badge */}
        <div className={`inline-flex items-center gap-1.5 border rounded-full px-2 py-0.5 mb-3 ${cfg.badge}`}>
        ...
        {proName && proInitials && (
          <div className="flex items-center gap-2 mb-2">
            <div className="w-6 h-6 rounded-full bg-amber-100 flex items-center justify-center flex-shrink-0">
              <span className="text-amber-700" style={{ fontSize: "9px", fontWeight: 800 }}>{proInitials}</span>
            </div>
            <span className="text-slate-600" style={{ fontSize: "11px", fontWeight: 500 }}>{proName}</span>
          </div>
        )}
        ...
        {showPrice && price && (
          <p className="text-slate-900 mb-2" style={{ fontSize: "18px", fontWeight: 800, lineHeight: 1.1 }}>
            ${price}
            <span className="text-slate-400 ms-1" style={{ fontSize: "11px", fontWeight: 400 }}>/hr</span>
          </p>
        )}
