...
const STATUS_CONFIG: Record<
  LeadStatus,
  { en: string; ar: string; dot: string; badge: string; text: string }
> = {
  pending: {
    en: 'Pending Bids',
    ar: 'قيد الانتظار',
    ...
    >
      ...
      <div className="flex-shrink-0 bg-white border-b border-slate-100 shadow-sm">
        ...
        <div className="flex gap-2 px-4 pb-3 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
          {filters.map((f) => {
            const count =
              f.key === 'all' ? leads.length : leads.filter((l) => l.status === f.key).length;
            ...
            {filtered.map((lead) => {
              ...
              return (
                ...
                >
                  {/* Icon */}
                  <div
                    className={`w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 ${svc.bg}`}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 mb-1.5">
                      ...
                      {/* Status badge */}
                      <span
                        className={`px-2.5 py-1 rounded-full border flex-shrink-0 ${st.badge} ${st.text}`}
