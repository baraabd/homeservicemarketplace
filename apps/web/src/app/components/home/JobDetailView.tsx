...
import {
  ChevronLeft, ChevronRight, MapPin, Calendar, Clock, Star,
  MessageCircle, Phone, Navigation, CheckCircle2, MoreVertical,
  Wrench, Zap, Wind, Sparkles, Hammer, PaintBucket,
  Shield, ThumbsUp, AlertCircle, XCircle,
} from "lucide-react";
...
function StatusTimeline({ status, lang }: { status: LeadStatus; lang: string }) {
  const steps = [
    {
      en: "Posted",        ar: "تم النشر",      doneAt: "9:00 AM",
      doneFills: ["pending","active","completed","cancelled"],
    },
    {
      en: "Bids Received", ar: "وصلت عروض",     doneAt: "9:15 AM",
      doneFills: ["active","completed"],
    },
    {
      en: "Pro Assigned",  ar: "تعيين محترف",   doneAt: "9:32 AM",
      doneFills: ["active","completed"],
    },
    {
      en: "In Progress",   ar: "جارٍ التنفيذ",  doneAt: "3:00 PM",
      doneFills: ["active","completed"],
    },
    {
      en: "Completed",     ar: "مكتمل",         doneAt: "5:10 PM",
      doneFills: ["completed"],
    },
  ];
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-4">
      <p className="text-slate-500 mb-4" style={{ fontSize: "11px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em" }}>
            ...
            <div key={i} className="relative flex items-start gap-4 mb-4 last:mb-0 z-10">
              ...
              <div
                className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 border-2 transition-all ${
                  isDone    ? "bg-amber-500 border-amber-500 shadow-sm shadow-amber-200" :
                  isCurrent ? "bg-white border-amber-500 ring-2 ring-amber-100"         :
              ...
              <div className="flex-1 pt-1.5">
                <div className="flex items-center justify-between">
                  ...
                  {isDone && (
                    <span className="text-slate-400" style={{ fontSize: "10px" }}>{step.doneAt}</span>
                  )}
                </div>
                {isCurrent && (
...
interface JobDetailViewProps {
  ...
  isVisible:  boolean;
  onBack:     () => void;
  onOpenChat: (contact: { name: string; initials: string; bg: string; textColor: string; status: string }) => void;
}
...
export function JobDetailView({ job, isVisible, onBack, onOpenChat }: JobDetailViewProps) {
  ...
  const displayAddress = (lang === "ar" && job.addressAr) ? job.addressAr : (job.address ?? (lang === "ar" ? "حي العليا، الرياض" : "Al Olaya District, Riyadh"));
  const displayDate    = (lang === "ar" && job.dateAr)    ? job.dateAr    : (job.date ?? (lang === "ar" ? "اليوم، 3:00 م" : "Today, 3:00 PM"));

  const statusColor =
    job.status === "active"    ? "bg-blue-50 border-blue-200 text-blue-700"   :
    job.status === "pending"   ? "bg-amber-50 border-amber-200 text-amber-700" :
    job.status === "completed" ? "bg-green-50 border-green-200 text-green-700" :
                                 "bg-red-50 border-red-200 text-red-700";

  const statusLabel =
    job.status === "active"    ? (lang === "ar" ? "جارٍ التنفيذ"    : "In Progress")  :
    job.status === "pending"   ? (lang === "ar" ? "قيد الانتظار"    : "Awaiting Bids") :
    ...
    <>
      ...
      >
        ...
        <div className="flex-shrink-0 bg-white border-b border-slate-100 shadow-sm">
          <div className="flex items-center gap-3 px-4 py-3.5">
            ...
            <div className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 ${svc.bg}`}>
            ...
            <div className="flex items-center gap-2">
              <span className={`px-2.5 py-1 rounded-full border ${statusColor}`} style={{ fontSize: "10px", fontWeight: 700 }}>
                {statusLabel}
              </span>
          ...
          {job.status === "active" && (
            <div className="bg-blue-500 rounded-2xl px-4 py-3 flex items-center gap-3">
              ...
              <p className="text-white flex-1" style={{ fontSize: "13px", fontWeight: 600 }}>
                {lang === "ar" ? "المحترف في طريقه إليك — وقت الوصول ≈ 15 دقيقة" : "Pro is on the way — ETA ~15 minutes"}
              </p>
            ...
            <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-4">
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1">
                  <p className="text-slate-900" style={{ fontSize: "16px", fontWeight: 800 }}>{job.proName}</p>
                  <div className="flex items-center gap-2 mt-0.5">
                    <Stars rating={job.proRating ?? 4.8} />
                    <span className="text-slate-500" style={{ fontSize: "11px" }}>
                      {job.proRating ?? 4.8} · {job.proReviews ?? 156} {lang === "ar" ? "تقييم" : "reviews"}
                    </span>
                  </div>
                ...
                {showHourlyRate && job.price && (
                  <div className="text-end">
                    <p className="text-slate-900" style={{ fontSize: "20px", fontWeight: 800 }}>${job.price}</p>
                    <p className="text-slate-400" style={{ fontSize: "10px" }}>/hr</p>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 mb-4">
                {(job.proTags ?? ["Licensed", "Insured", "Top Rated"]).map((tag) => (
                  <span key={tag} className="px-2.5 py-1 rounded-xl bg-slate-100 text-slate-600" style={{ fontSize: "11px", fontWeight: 600 }}>
                    {tag}
                  </span>
                ))}
              </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-4">
            ...
            {[
              ...
              <div key={row.label} className="flex items-center gap-3 py-2.5 border-b border-slate-50 last:border-0">
                ...
                <span className="text-slate-400 w-20 flex-shrink-0" style={{ fontSize: "12px" }}>{row.label}</span>
                <span className="text-slate-900 flex-1" style={{ fontSize: "12px", fontWeight: 600 }}>{row.val}</span>
              </div>
            ))}
          </div>
          ...
          {showHourlyRate && job.price && (
            <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-4">
              ...
              {[
                ...
                { label: lang === "ar" ? "المدة التقديرية"  : "Est. duration",      val: lang === "ar" ? "2 ساعة" : "~2 hours"  },
                { label: lang === "ar" ? "رسوم الخدمة (5%)" : "Service fee (5%)",  val: `$${Math.round(job.price * 0.1)}`       },
              ].map((row, i) => (
                <div key={i} className="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
                  <span className="text-slate-500" style={{ fontSize: "12px" }}>{row.label}</span>
                  <span className="text-slate-700" style={{ fontSize: "12px", fontWeight: 600 }}>{row.val}</span>
                </div>
              ))}
            ...
            >
              ...
              <div className="flex-1 text-start">
                ...
                <p className="text-white/70" style={{ fontSize: "12px" }}>
                  {lang === "ar" ? "اترك تقييماً لمساعدة المحترفين الآخرين" : "Leave a review to help other pros"}
                </p>
              </div>
      ...
      {showRateModal && (
        <div className="absolute inset-0 z-30 flex flex-col justify-end">
          <div className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm" onClick={() => setShowRateModal(false)} />
          <div className="relative bg-white rounded-t-3xl px-6 py-6">
            ...
            <div className="flex flex-col items-center text-center gap-2 mb-6">
              ...
              <p className="text-slate-900" style={{ fontSize: "18px", fontWeight: 800 }}>
                {lang === "ar" ? `كيف كانت تجربتك مع ${job.proName}؟` : `Rate your experience with ${job.proName}`}
              </p>
            ...
            <div className="flex justify-center gap-3 mb-6">
              {[1,2,3,4,5].map((s) => (
                <button key={s} onClick={() => setSelectedRating(s)} className="active:scale-90 transition-all">
            ...
            <button
              ...
              className={`w-full py-4 rounded-2xl transition-all active:scale-95 ${
                selectedRating > 0 ? "bg-amber-500 text-white shadow-md shadow-amber-200" : "bg-slate-100 text-slate-400"
              }`}
