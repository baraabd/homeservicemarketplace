...
import {
  ChevronLeft, ChevronRight, Clock, TrendingDown, Star, Zap,
  RefreshCw, CheckCircle2, Filter,
} from "lucide-react";
...
const SEED_BIDS: Bid[] = [
  {
    id: "b1", name: "Omar Al-Khalid",   initials: "OK",
    avatarBg: "bg-amber-100", avatarColor: "text-amber-700",
    rating: 4.9, reviewCount: 312, jobCount: 540, price: 35,
    tags: ["Licensed", "Insured", "Top Rated"], verified: true,  topPro: true,
    responseTime: "within 5 min",  badge: "bestMatch",
  },
  {
    id: "b2", name: "Khalid Hassan",    initials: "KH",
    avatarBg: "bg-blue-100",  avatarColor: "text-blue-700",
    rating: 4.7, reviewCount: 156, jobCount: 220, price: 28,
    tags: ["Budget-Friendly", "Quick Response"], verified: true, topPro: false,
    responseTime: "within 20 min", badge: "bestValue",
  },
  {
    id: "b3", name: "Ali Al-Rashid",    initials: "AR",
    avatarBg: "bg-green-100", avatarColor: "text-green-700",
    rating: 4.6, reviewCount: 89,  jobCount: 180, price: 30,
    tags: ["Available Now", "Fast"], verified: true, topPro: false,
    responseTime: "within 10 min", badge: "fastest",
  },
  {
    id: "b4", name: "Mohammed Al-Zahra",initials: "MZ",
    avatarBg: "bg-purple-100",avatarColor: "text-purple-700",
    rating: 4.8, reviewCount: 67,  jobCount: 145, price: 40,
    tags: ["Premium Service", "Certified"], verified: false, topPro: false,
    responseTime: "within 30 min", badge: null,
  },
  {
    id: "b5", name: "Hassan Mustafa",   initials: "HM",
    avatarBg: "bg-slate-100", avatarColor: "text-slate-700",
    rating: 4.5, reviewCount: 42,  jobCount: 78,  price: 25,
    tags: ["New Pro", "Eco Products"], verified: false, topPro: false,
    responseTime: "within 45 min", badge: null,
  },
];

const LIVE_BID: Bid = {
  id: "b6", name: "Faisal Al-Nasser",  initials: "FN",
  avatarBg: "bg-red-100",   avatarColor: "text-red-700",
  rating: 4.8, reviewCount: 189, jobCount: 310, price: 32,
  tags: ["Top Rated", "Lightning Fast"], verified: true, topPro: false,
  responseTime: "within 15 min", badge: null,
};
  ...
  return (
    <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
      ...
      <div className="flex justify-between mt-2">
        <span className="text-green-600" style={{ fontSize: "11px", fontWeight: 700 }}>${min} low</span>
        <span className="text-slate-400" style={{ fontSize: "10px" }}>avg ${avg}</span>
        <span className="text-red-400"   style={{ fontSize: "11px", fontWeight: 700 }}>${max} high</span>
      </div>
    </div>
  );
}
        ...
        {newBidPending && (
          <div className="flex items-center gap-3 bg-blue-50 border border-blue-200 rounded-2xl px-4 py-3 my-3">
            <div className="w-7 h-7 rounded-xl bg-blue-500 flex items-center justify-center flex-shrink-0">
              <RefreshCw size={13} className="text-white animate-spin" style={{ animationDuration: "1.2s" }} />
            </div>
            ...
            return (
              <div
                ...
                className={`relative transition-all duration-500 ${
                  newBidFlashId === bid.id ? "ring-2 ring-blue-400 rounded-3xl shadow-lg shadow-blue-100" : ""
                }`}
              ...
              >
                {badgeCfg && (
                  <div className="absolute -top-3 start-4 z-10 flex items-center gap-1">
                    <div className={`${badgeCfg.bg} ${badgeCfg.text} px-3 py-1 rounded-full shadow-sm`} style={{ fontSize: "10px", fontWeight: 800 }}>
                      {badgeCfg.label}
                    </div>
                  </div>
                )}
                ...
                {bid.id === LIVE_BID.id && (
                  <div className="absolute -top-3 end-4 z-10">
                    <div className="bg-blue-500 text-white px-2.5 py-1 rounded-full flex items-center gap-1" style={{ fontSize: "10px", fontWeight: 800 }}>
                ...
                <div className={`transition-all duration-300 ${badgeCfg ? "mt-3" : ""} ${isAccepted ? "scale-95 opacity-60" : ""}`}>
                  <ProBidCard
                    name={bid.name}
            ...
            <table className="w-full" style={{ fontSize: "11px" }}>
              <thead>
                <tr className="text-slate-400">
                  ...
                  <th className="text-center py-2 px-2 font-semibold">{lang === "ar" ? "التقييم" : "Rating"}</th>
                  {showHourlyRate && <th className="text-center py-2 px-2 font-semibold">{lang === "ar" ? "السعر" : "Price"}</th>}
                  <th className="text-center py-2 px-2 font-semibold">{lang === "ar" ? "الردّ" : "Response"}</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={bid.id} className="border-t border-slate-50">
                    <td className="py-2 pe-3">
                      <div className="flex items-center gap-2">
                        <div className={`w-6 h-6 rounded-lg flex items-center justify-center ${bid.avatarBg}`}>
                          <span className={bid.avatarColor} style={{ fontSize: "8px", fontWeight: 800 }}>{bid.initials}</span>
                        </div>
                        <span className="text-slate-700 font-semibold truncate" style={{ maxWidth: "70px" }}>{bid.name.split(" ")[0]}</span>
                      </div>
                    </td>
                    ...
                    <td className="text-center py-2 px-2 text-slate-500">{bid.responseTime.replace("within ", "")}</td>
                  </tr>
                ))}
              </tbody>
            </table>
