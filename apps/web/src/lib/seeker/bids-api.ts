...
// `withCredentials: true`); mutations pick up the X-CSRF-Token
...
export async function acceptBid(requestId: string, bidId: string): Promise<AcceptBidResponse> {
  const { data } = await api.post<AcceptBidResponse>(
